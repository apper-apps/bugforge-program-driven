...
try {
      ...
      if (response.results) {
        ...
        const createdReply = successfulRecords.length > 0 ? successfulRecords[0].data : null;
        
        // Process mentions in reply text
        if (createdReply && replyData.text) {
          await this.processMentions(replyData.text, createdReply.Id, replyData.authorId);
        }
  ...
  async processMentions(text, replyId, authorId) {
    // Enhanced mention detection for replies
      ...
      for (const mention of mentions) {
        ...
        try {
          await notificationService.createForMention(
            ...
            null, // Reply doesn't have direct comment_id_c relation in notifications
            'System' // Should be actual author name
          );
        } catch (error) {
          console.error('Failed to create mention notification for reply:', error);
        }
      }
    }
  },

  async edit(id, replyData) {
    try {
      const { ApperClient } = window.ApperSDK;
      const apperClient = new ApperClient({
        apperProjectId: import.meta.env.VITE_APPER_PROJECT_ID,
