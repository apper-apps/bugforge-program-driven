...
steps: (() => {
  if (!testCase.steps_c) return [""];
  if (Array.isArray(testCase.steps_c)) return testCase.steps_c;
  if (typeof testCase.steps_c === 'string') {
    try {
      const parsed = JSON.parse(testCase.steps_c);
      return Array.isArray(parsed) ? parsed : [testCase.steps_c];
    } catch {
      return [testCase.steps_c];
    }
  }
  return [""];
})(),
